...
// adding telemetry collection that gets pushed to kril via gRPC.
//
// Usage:
//
...
import (
	...
	pb "github.com/Rajneesh100/kril/backend/pkg/telemetry/pb"
...
// Configure sets the service name and kril gRPC address.
...
func getClient() pb.TelemetryServiceClient {
	...
	if err != nil {
		log.Printf("kril ctx: failed to connect to %s: %v", globalGRPCAddr, err)
		return nil
	}
	globalConn = conn
...
// --- KrilCtx: the main context wrapper ---
...
// Options for creating a new KrilCtx.
type Option func(*KrilCtx)

func WithProcessType(pt string) Option { return func(s *KrilCtx) { s.processType = pt } }
func WithEndpoint(ep string) Option    { return func(s *KrilCtx) { s.endpoint = ep } }

// New creates a new KrilCtx wrapping the given context.
func New(parent context.Context, opts ...Option) *KrilCtx {
	sc := &KrilCtx{
		Context:      parent,
		isSuccessful: true,
		startTime:    time.Now(),
...
func (sc *KrilCtx) Trace(name string, fn func(tc *TraceCtx) (any, error)) (any, error) {
	return sc.traceWithParent(name, "", fn)
}

// TraceChild is like Trace but explicitly sets the parent function name.
func (sc *KrilCtx) TraceChild(name, parentName string, fn func(tc *TraceCtx) (any, error)) (any, error) {
	return sc.traceWithParent(name, parentName, fn)
}

func (sc *KrilCtx) traceWithParent(name, parentName string, fn func(tc *TraceCtx) (any, error)) (any, error) {
	if name == "" {
		name = callerName(2)
	}
...
func (sc *KrilCtx) TraceParallel(parentName string, fns map[string]func(tc *TraceCtx) (any, error)) map[string]any {
	idx := sc.indexCounter.Add(1) - 1
...
func (sc *KrilCtx) MarkFailed() {
	sc.isSuccessful = false
}

// Finish completes the context and asynchronously pushes telemetry to kril.
// Always call this via defer.
func (sc *KrilCtx) Finish() {
	...
	go func() {
		...
		if _, err := client.PushMetric(bgCtx, req); err != nil {
			log.Printf("kril ctx: push metric failed: %v", err)
		}
	}()
}
