...
//   starfish_request_total        — counter per service/endpoint/status
//   starfish_request_latency_ms   — gauge per service/endpoint
//   starfish_method_error_total   — counter per service/endpoint/function
//   starfish_function_latency_ms  — gauge per service/function
package victoria
...
func (c *Client) PushMetrics(ctx context.Context, tlog *storage.TelemetryLog) error {
	...
	lines = append(lines, fmt.Sprintf(
		`starfish_request_total{service=%q,endpoint=%q,process_type=%q,status=%q} 1 %d`,
		tlog.ServiceName, tlog.Endpoint, tlog.ProcessType, status, ts,
	))

	// Request latency
	lines = append(lines, fmt.Sprintf(
		`starfish_request_latency_ms{service=%q,endpoint=%q,process_type=%q} %f %d`,
	...
	if tlog.MethodFailure {
		lines = append(lines, fmt.Sprintf(
			`starfish_method_failure_total{service=%q,endpoint=%q} 1 %d`,
	...
	for _, entry := range tlog.ExecutionMap {
		lines = append(lines, fmt.Sprintf(
			`starfish_function_latency_ms{service=%q,function=%q,endpoint=%q} %f %d`,
			tlog.ServiceName, entry.CurrentFunction, tlog.Endpoint, entry.LatencyMs, ts,
		))

		if entry.HaveError {
			lines = append(lines, fmt.Sprintf(
				`starfish_function_error_total{service=%q,function=%q,endpoint=%q} 1 %d`,
		...
		for _, ec := range entry.ExternalCalls {
			...
			lines = append(lines, fmt.Sprintf(
				`starfish_external_call_total{service=%q,function=%q,target=%q,call_type=%q} 1 %d`,
